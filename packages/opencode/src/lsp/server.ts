      ...
      if (!bin) {
        ...
        if (!cs) {
          ...
          if (!(await Bun.file(cs).exists())) {
            ...
            const temp = path.join(Global.Path.bin, `cs.${ext}`)
            await Bun.file(temp).write(response)

            if (ext === "zip") {
              const ok = await Archive.extractZip(temp, Global.Path.bin)
                .then(() => true)
                .catch((error) => {
        ...
        const proc = Bun.spawn({
          cmd: [cs, "install", "metals", "--install-dir", Global.Path.bin],
          stdout: "pipe",
          stderr: "pipe",
          stdin: "pipe",
        })
        ...
        bin = path.join(Global.Path.bin, "metals" + (process.platform === "win32" ? ".bat" : ""))
